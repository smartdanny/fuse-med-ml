...
def run_train(paths, train_params):
    ...
    local_rank = train_params["LOCAL_RANK"]
    # log clearml only on rank 0
    if (int(node_rank) == 0) & (int(local_rank) == 0):
        print("✅ INITIALIZING CLEARML ✅")
        task = Task.init(
            project_name="shatz_root/shatz_ddp_mnist_7",
            task_name=f"pyutils-launch num_nodes={num_nodes} num_gpus={num_gpus} bs={bs} lr={lr}",
        )
        # else:
        #     print(f"✅ REUSING TASK node_rank: {node_rank} || local_rank: {local_rank}")
        #     task = curr_task
...
if __name__ == "__main__":
    ...
    TRAIN_PARAMS["LOCAL_RANK"] = args.local_rank
    use_ddp = False
